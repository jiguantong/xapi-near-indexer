...
export class PublisherStarter {
  ...
  async start(options: StartOptions) {
    this._nearGraphqlEndpoint = XAPIConfig.graphql.endpoint(
      options.testnet ? "near-testnet" : "near",
    );
      ...
      for (const aggregator of this._aggregators) {
        for (const chainId of aggregator.supported_chains) {
          ...
          try {
            logger.info("------------------------------------------------");
            logger.info(
              `==== 📞 start config-syncer for ${aggregator.id} [${
                chain.name
              }-${chain.id.toString()}] ====`,
              {
          ...
          try {
            logger.info("------------------------------------------------");
            logger.info(
              `==== 📦 start publisher for ${aggregator.id} [${
                chain.name
              }-${chain.id.toString()}] ====`,
              {
                target: "publisher",
  ...
  async runPublisher(lifecycle: PublisherLifecycle) {
    ...
    logger.info(
      `==> ${lifecycle.aggregator} [${
        targetChain.name
      }-${targetChain.id.toString()}] toPublishIds: [${toPublishIds.length}]`,
      {
        target: "publisher",
      },
    );
    logger.debug(
      `==> ${lifecycle.aggregator} [${
        targetChain.name
      }-${targetChain.id.toString()}] toPublishIds: ${toPublishIds}`,
      {
        target: "publisher",
      },
    );
    ...
    for (const aggregated of aggregatedEvents) {
      ...
      const checkCache = await lifecycle.cache.get(
        this.publishCacheKey(lifecycle.aggregator, relatedRequest!.requestId),
      );
      if (checkCache) {
        logger.warn(
          `==> ${lifecycle.aggregator} [${
            targetChain.name
          }-${targetChain.id.toString()}] skip cached request: ${
      ...
      logger.info(
        `==> ${lifecycle.aggregator} [${
          targetChain.name
        }-${targetChain.id.toString()}] double check ${relatedRequest?.requestId}, status: ${
          _request.status
        }`,
        {
          target: "publisher",
      ...
      if (_request.status == 0) {
        ...
        logger.info(
          `==> ${lifecycle.aggregator} [${relatedRequest?.requestId}] triggerPublish`,
          {
            target: "publisher",
          },
        );
  ...
  async runConfigSyncer(lifecycle: PublisherLifecycle) {
    ...
    if (!latestConfigFromNear) {
      logger.warn(
        `==> [${
          targetChain.name
        }-${targetChain.id.toString()}] No publish config for ${
          lifecycle.aggregator
        }`,
        {
          target: "config-syncer",
    ...
    const latestConfigFromEvm =
      await this.evmGraphqlService.queryAggregatorConfig({
        endpoint: XAPIConfig.graphql.endpoint(targetChain.code),
        aggregator: lifecycle.aggregator,
    ...
    if (
      latestConfigFromEvm &&
      latestConfigFromEvm.version >= latestConfigFromNear.version
    ) {
      logger.info(
        `==> [${targetChain.name}-${targetChain.id.toString()}] ${
          lifecycle.aggregator
        }, 1-check near: ${
          latestConfigFromNear.version
        } <= evm: ${latestConfigFromEvm?.version}`,
        {
          target: "config-syncer",
        },
      );
    ...
    const exAggregator = await this.deriveXAPIAddress(
      lifecycle.aggregator,
      lifecycle,
    );
    const aggregatorConfigEvm =
      await lifecycle.nearEthereum.getContractViewFunction(
        latestConfigFromNear.xapi_address,
        xapiAbi,
        "aggregatorConfigs",
        [exAggregator],
      );
    if (
      aggregatorConfigEvm &&
      aggregatorConfigEvm.version >= latestConfigFromNear.version
    ) {
      logger.info(
        `==> [${targetChain.name}-${targetChain.id.toString()}] ${
          lifecycle.aggregator
          ...
          latestConfigFromNear.version
        } <= evm: ${aggregatorConfigEvm?.version}`,
        {
          target: "config-syncer",
        },
      );
    ...
    const checkCache = await lifecycle.cache.get(
      this.syncConfigCacheKey(
        lifecycle.aggregator,
        lifecycle.targetChain.id,
        latestConfigFromNear.version,
      ),
    );
    if (checkCache) {
      logger.warn(
        `==> ${lifecycle.aggregator} [${
          targetChain.name
        }-${targetChain.id.toString()}] skip cached config version: ${
          latestConfigFromNear.version
        }, ${checkCache}`,
        {
          target: "config-syncer",
  ...
  async triggerPublish(
    aggregated: XAPIResponse,
    relatedRequest: RequestMade,
    lifecycle: PublisherLifecycle,
  ) {
    // Derive address
    const deriveAddress = await this.deriveXAPIAddress(
    ...
    try {
      gasLimit = await lifecycle.nearEthereum.estimateGas(
        relatedRequest.xapiAddress,
        xapiAbi,
        "fulfill",
        [
          relatedRequest.requestId,
          [
            aggregated.reporter_reward_addresses,
            lifecycle.nearEthereum.stringToBytes(aggregated.result),
    ...
    maxPriorityFeePerGas = (
      (BigInt(maxPriorityFeePerGas) * BigInt(3)) /
      BigInt(2)
    ).toString();
    logger.info(
    ...
    try {
      ...
      if (e.message.includes("Exceeded the prepaid gas")) {
        await lifecycle.cache.put(
          this.publishCacheKey(lifecycle.aggregator, relatedRequest.requestId),
          `Exceeded the prepaid gas.`,
        );
      } else {
        await setTimeout(5000);
        result = await this.nearGraphqlService.queryPublishSignature({
          endpoint: this._nearGraphqlEndpoint!,
    ...
    if (result && result.signature) {
      ...
      try {
        await this.relayMpcTx(
          result.response.chain_id,
          Address.fromString(result.publish_chain_config.xapi_address),
          result.call_data,
          {
            id: "0",
            // @ts-ignore
            big_r_affine_point:
              _signature.big_r_affine_point || _signature.big_r.affine_point,
            // @ts-ignore
      ...
      logger.error(
        `===> publish_external error: can't find result from indexer, aggregator: ${lifecycle.aggregator}, request_id: ${relatedRequest.requestId}`,
        {
          target: "do-publish",
        },
      );
  ...
  async triggerSyncConfig(
    ...
    try {
      gasLimit = await lifecycle.nearEthereum.estimateGas(
        publishChainConfig.xapi_address,
        xapiAbi,
        "setAggregatorConfig",
        [
          publishChainConfig.aggregator,
          publishChainConfig.reporters_fee,
          publishChainConfig.publish_fee,
          publishChainConfig.reward_address,
          publishChainConfig.version,
        ],
        deriveAddress,
      );
    } catch (e: any) {
    ...
    try {
      ...
      if (e.message.includes("Exceeded the prepaid gas")) {
        ...
        await setTimeout(5000);
        result = await this.nearGraphqlService.querySyncConfigSignature({
          endpoint: this._nearGraphqlEndpoint!,
          chainId: lifecycle.targetChain.id.toString(),
    ...
    if (result && result.signature) {
      ...
      logger.error(
        `===> sync_publish_config_to_remote error: can't find result from indexer, aggregator: ${
          lifecycle.aggregator
        }, chain_id: ${lifecycle.targetChain.id.toString()}, version: ${
          publishChainConfig.version
        }`,
        {
          target: "do-sync-config",
  ...
  async relayMpcTx(
    ...
    logger.info(
      `===> [${lifecycle.targetChain.name}-${lifecycle.targetChain.id}] tx: ${txHash} `,
      {
        target: " 🚀 ",
      },
    );
  ...
  async deriveXAPIAddress(aggregator: string, lifecycle: PublisherLifecycle) {
    return (
      await lifecycle.nearEthereum.deriveAddress(
        aggregator,
        `XAPI-${lifecycle.targetChain.id.toString()}`,
      )
    ).address;
  }

  async checkAggregatorBalance(
    aggregator: string,
    lifecycle: PublisherLifecycle,
  ): Promise<boolean> {
    ...
    try {
      ...
      if (_signerDepositRequired) {
        signerDepositRequired = new Decimal(_signerDepositRequired).div(
          new Decimal("1000000000000000000000000"),
        );
        if (
          BigInt(_signerDepositRequired) > BigInt(mpcConfig.attached_balance)
        ) {
          logger.warn(
            `==== ❌ MpcConfig attached_balance of ${aggregator}: ${mpcConfig.attached_balance} < ${_signerDepositRequired}, break ====`,
            {
              target: "check-aggregator-balance",
            },
          );
    ...
    const aggregatorAccount = new NearAccount(
      lifecycle.near.near.connection,
      aggregator,
    );
    const balance = await aggregatorAccount.getAccountBalance();
    const availableBalance = new Decimal(balance.available).div(
      new Decimal("1000000000000000000000000"),
    );
    if (
      availableBalance.comparedTo(
        MINIMUM_AGGREGATOR_NEAR.add(new Decimal(signerDepositRequired)),
      ) < 0
    ) {
      logger.warn(
        `==== ❌ Avaliable Balance of ${aggregator}: ${availableBalance.toFixed(
          4,
        )} NEAR < (${MINIMUM_AGGREGATOR_NEAR} + ${signerDepositRequired}) NEAR, break ====`,
        {
          target: "check-aggregator-balance",
        },
      );
      ...
      logger.info(
        `==== ✅ Avaliable Balance of ${aggregator}: ${availableBalance.toFixed(
          4,
        )} NEAR >= (${MINIMUM_AGGREGATOR_NEAR} + ${signerDepositRequired}) NEAR ====`,
        {
          target: "check-aggregator-balance",
        },
      );
