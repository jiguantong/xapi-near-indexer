...
export function handleAggregatorConfigSet(
  ...
  entity.aggregator = event.params.aggregator.toHex().toLowerCase()
  entity.perReporterFee = event.params.perReporterFee
  entity.publishFee = event.params.publishFee
  entity.fulfillAddress = event.params.fulfillAddress.toHex().toLowerCase()
  entity.rewardAddress = event.params.rewardAddress.toHex().toLowerCase()
...
export function handleFulfilled(event: FulfilledEvent): void {
  ...
  entity.response_reporters = event.params.response.reporters.map(reporter => reporter.toHex().toLowerCase())
  entity.response_result = event.params.response.result.toHex().toLowerCase()
...
export function handleOwnershipTransferStarted(
  ...
  entity.previousOwner = event.params.previousOwner.toHex().toLowerCase()
  entity.newOwner = event.params.newOwner.toHex().toLowerCase()
...
export function handleRequestMade(event: RequestMadeEvent): void {
  ...
  entity.requester = event.params.requester.toHex().toLowerCase()
...
export function handleRewardsWithdrawn(event: RewardsWithdrawnEvent): void {
  ...
  entity.withdrawer = event.params.withdrawer.toHex().toLowerCase()
